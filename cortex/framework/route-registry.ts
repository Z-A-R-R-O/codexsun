...
export class RouteRegistry {
    ...
    fallback(handler: Route["handler"], options: Partial<Route> = {}) {
        this.fallbackRoute = {method: "ANY", path: "*", handler, params: [], ...options};
        ...
        for (const cached of this.cachedConfigs.sort((a, b) => b.normalizedPath.length - a.normalizedPath.length)) {
            ...
            for (const route of cached.normalizedRoutes) {
                ...
                if (params) {
                    if (route.rateLimit && !(await this.checkRateLimit(route, path, userId, ip))) {
                        return {
                            route: {
                                method: "ANY",
                                path,
                                handler: async () => ({status: 429, body: {error: "Rate limit exceeded"}})
                            },
                            config,
                            params: {},
                        };
                        ...
                        try {
                            ...
                            return {
                                route: {
                                    method: "ANY",
                                    path,
                                    handler: async () => ({status: 500, body: {error: "Model resolution failed"}})
                                },
                                config,
                                params: {},
                            };
                    ...
                    return {route, config, params, model};
                }
            }
                                ...
                                try {
                                    ...
                                    return {
                                        route: {
                                            ...
                                            handler: async () => ({
                                                status: 500,
                                                body: {error: "Model resolution failed"}
                                            })
                                        },
                                        config: normalizedSubConfig.config,
                                        params: {},
