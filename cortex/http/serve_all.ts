...
import type { LoggerOptions } from "./chttpx";
...
export interface BootOptions {
    ...
    tlsCertPath?: string;
    tlsCaPath?: string;
    cors?: CORSOptions | true; // pass-through to chttpx (defaults to ENV if omitted)
    sftp?: {
        enable?: boolean;
        port?: number;
        hostKeyPath?: string;
    ...
    logger?: LoggerOptions; // structured access/error logs
}
...
export async function bootAll(opts: BootOptions) {
    ...
    const keyPath =
        opts.tlsKeyPath ?? process.env.TLS_KEY ?? path.resolve(process.cwd(), "server.key");
    const certPath =
        opts.tlsCertPath ?? process.env.TLS_CERT ?? path.resolve(process.cwd(), "server.crt");
    ...
    if (sftpCfg.enable ?? true) {
        ...
        const sshKeyPath =
            ...
            path.resolve(process.cwd(), "ssh_host_ed25519_key");
        const sftpRoot =
            sftpCfg.rootDir ?? process.env.SFTP_ROOT ?? path.resolve(process.cwd(), "sftp-root");
...
export async function bootAllFromRegistry(
    registry: RouteRegistry,
    opts?: Omit<BootOptions, "providers">
) {
